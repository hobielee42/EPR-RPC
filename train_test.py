...
if __name__ == "__main__":
    ...
    mode, num_epochs, continue_ = "local", 1, False
    split = int(get_args())
    ...
    len_ds = len(train_ds)
    split_size = len_ds // 10
    ds_split = train_ds.select(range((split - 1) * split_size, split * split_size))
    # test_ds: Dataset = test_tokens.add_column("alignment", test_alignments).with_format(
    #     "torch"
    # )
    train_dl = DataLoader(ds_split)
    ...
    if not continue_:
        ...
        num_training_steps = len(train_dl) // batch_size * num_epochs
    ...
    for epoch in range(epoch_start, num_epochs):
        train_acc = test_epoch(model, train_dl, optimizer, scheduler)
